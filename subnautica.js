...
module.exports.run = async function ({
    ...
    case '-r': {
        ...
        if (!existsSync(pathData)) {
            ...
            obj.fishBag.push({
                ID: 0,
                name: 'Cá Thách Đấu | Không được bán',
                category: 'Legendary',
                size: 999999,
                sell: 0
            });
            writeFileSync(pathData, JSON.stringify(obj, null, 4));
            var msg = {body: "(¯´•.•CÂU CÁ TRÊN 4546B•.•´¯)\n⚔️Đăng kí database thành công⚔️\nNhập ,subn shop để mua cần câu\nLưu ý nên để lại ít nhất 1 con cá trong inventory để hệ thống không bị lỗi", attachment: await this.subnautica()}
...
module.exports.getFish = function () {
    ...
    if (rate > 5 && rate <= 3500) return 'Common';
    if (rate > 3500 && rate <= 6000) return 'Uncommon';
    if (rate > 6000 && rate <= 8000) return 'Rare';
    if (rate > 8000 && rate <= 9500) return 'Epic';
    if (rate > 9500 && rate <= 9999) return 'Legendary';
    if (rate > 9999 && rate <= 10000) return 'Mythical';
}
